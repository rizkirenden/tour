...
@section('title', 'Produk Paket - Arrum Tour')
@section('page-title', 'Produk Paket')
...
@section('breadcrumb')
    ...
    <li class="inline-flex items-center">
        <i class="fas fa-chevron-right text-gray-400 mx-2 text-xs"></i>
        <span class="text-gray-500 font-medium">Produk Paket</span>
    </li>
@endsection
...
@section('content')
    <div class="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
        <div class="px-6 py-4 border-b border-gray-200 flex flex-wrap items-center justify-between gap-3">
            <div>
                <h5 class="text-sm font-semibold text-gray-700">Daftar Produk Paket</h5>
                <p class="text-xs text-gray-400 mt-0.5">Kelola semua produk paket umroh</p>
            </div>
            <div class="flex items-center gap-3">
                <span class="text-xs text-gray-500">
                    Total: <span class="font-medium text-gray-700">{{ $data->total() }}</span> produk
                </span>
...
@push('scripts')
    <script>
        function applyFilter() {
            ...
            fetch(`{{ route('master.produk.index') }}?search=${search}&ajax=true`)
