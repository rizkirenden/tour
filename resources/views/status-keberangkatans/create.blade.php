...
@section('title', 'Tambah Status Keberangkatan - Arrum Tour')
@section('page-title', 'Tambah Status Keberangkatan')
    ...
    <div class="mx-auto">
        <div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
            <div class="px-6 py-4 border-b border-gray-100">
                <h5 class="text-sm font-semibold text-gray-700">Form Tambah Status Keberangkatan</h5>
            </div>

            <form action="{{ route('master.status-keberangkatan.store') }}" method="POST">
                @csrf
                ...
                <div class="p-6 space-y-6">
                    ...
                    <div>
                        ...
                        <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-3">
                            @foreach ($warnaOptions as $key => $label)
                                <label
                                    class="flex items-center gap-2 p-3 border border-gray-200 rounded-lg hover:bg-gray-50 cursor-pointer transition {{ old('warna') == $key ? 'border-yellow-500 bg-yellow-50' : '' }}">
                                    <input type="radio" name="warna" value="{{ $key }}"
                                        {{ old('warna') == $key ? 'checked' : '' }}
                    ...
                    <div>
                        ...
                        <textarea name="keterangan" rows="3"
                            class="w-full px-4 py-2 border border-gray-200 rounded-xl focus:ring-2 focus:ring-yellow-500 focus:border-yellow-500 text-sm"
                            placeholder="Deskripsikan status ini...">{{ old('keterangan') }}</textarea>
                        @error('keterangan')
                            <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                        @enderror
                    </div>
