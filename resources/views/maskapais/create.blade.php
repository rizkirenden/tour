...
@section('title', 'Tambah Maskapai - Arrum Tour')
    ...
    <div class="mx-auto">
        <div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
            <div class="px-6 py-4 border-b border-gray-100">
                <h5 class="text-sm font-semibold text-gray-700">Form Tambah Maskapai</h5>
                <p class="text-xs text-gray-400 mt-0.5">Pilih minimal 1 tipe penerbangan</p>
            </div>

            <form action="{{ route('master.maskapai.store') }}" method="POST">
                @csrf
                ...
                <div class="p-6 space-y-6">
                    <div>
                        ...
                        <input type="text" name="nama_maskapai" value="{{ old('nama_maskapai') }}"
                    ...
                    <div>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-3">
                            <label
                                class="flex items-center p-4 border border-gray-200 rounded-xl hover:bg-green-50 transition-colors cursor-pointer {{ in_array('Domestik', old('tipe_penerbangan', [])) ? 'border-green-500 bg-green-50' : '' }}">
                                <input type="checkbox" name="tipe_penerbangan[]" value="Domestik"
                                    {{ in_array('Domestik', old('tipe_penerbangan', [])) ? 'checked' : '' }}
                            ...
                            <label
                                class="flex items-center p-4 border border-gray-200 rounded-xl hover:bg-blue-50 transition-colors cursor-pointer {{ in_array('Internasional', old('tipe_penerbangan', [])) ? 'border-blue-500 bg-blue-50' : '' }}">
                                <input type="checkbox" name="tipe_penerbangan[]" value="Internasional"
                                    {{ in_array('Internasional', old('tipe_penerbangan', [])) ? 'checked' : '' }}
