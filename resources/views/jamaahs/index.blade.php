...
@section('breadcrumb')
    ...
    <li class="inline-flex items-center">
        <i class="fas fa-chevron-right text-gray-400 mx-2 text-xs"></i>
        <span class="text-gray-500 font-medium">Jamaah</span>
    </li>
@endsection
...
@section('content')
    <div class="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
        <div class="px-6 py-4 border-b border-gray-200 flex flex-wrap items-center justify-between gap-3">
            <div>
                <h5 class="text-sm font-semibold text-gray-700">Daftar Jamaah</h5>
                <p class="text-xs text-gray-400 mt-0.5">Kelola semua data jamaah</p>
            </div>
            <div class="flex items-center gap-3">
                <span class="text-xs text-gray-500">
                    Total: <span class="font-medium text-gray-700">{{ $data->total() }}</span> jamaah
                </span>
                <a href="{{ route('transaksional.jamaah.create') }}"
        ...
        <div class="p-6">
            <form method="GET" action="{{ route('transaksional.jamaah.index') }}" id="filterForm" class="mb-4">
                <div class="grid grid-cols-1 md:grid-cols-4 gap-3">
                    ...
                    <div>
                        <select name="jenis_kelamin" id="filterGender"
                            class="w-full px-4 py-2 border border-gray-200 rounded-lg focus:ring-2 focus:ring-yellow-500 focus:border-yellow-500 text-sm">
