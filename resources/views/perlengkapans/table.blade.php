        ...
        <thead>
            <tr class="border-b border-gray-200">
                ...
                <th class="px-4 py-3 text-right text-xs font-semibold text-gray-500 uppercase tracking-wider">Harga
                    Satuan</th>
                <th class="px-4 py-3 text-center text-xs font-semibold text-gray-500 uppercase tracking-wider">Satuan
                </th>
                ...
                <tr class="hover:bg-gray-50 transition-colors duration-150">
                    ...
                    <td class="px-4 py-3">
                        <div class="flex items-center justify-center gap-1.5">
                            <a href="{{ route('master.perlengkapan.show', $item->id_perlengkapan) }}"
                                class="p-2 text-blue-500 hover:bg-blue-50 rounded-lg transition-all duration-200">
                                <i class="fas fa-eye text-sm"></i>
                            </a>
                            <a href="{{ route('master.perlengkapan.edit', $item->id_perlengkapan) }}"
                                class="p-2 text-amber-500 hover:bg-amber-50 rounded-lg transition-all duration-200">
                                <i class="fas fa-edit text-sm"></i>
                            </a>
                            <button onclick="confirmDelete({{ $item->id_perlengkapan }})"
                                class="p-2 text-red-500 hover:bg-red-50 rounded-lg transition-all duration-200">
                                <i class="fas fa-trash text-sm"></i>
                            </button>
                            <form id="delete-form-{{ $item->id_perlengkapan }}"
                                action="{{ route('master.perlengkapan.destroy', $item->id_perlengkapan) }}"
...
@push('scripts')
    <script>
        function confirmDelete(id) {
            if (confirm('Yakin ingin menghapus perlengkapan ini?')) {
