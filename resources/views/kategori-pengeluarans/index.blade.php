...
@section('title', 'Kategori Pengeluaran - Arrum Tour')
...
@section('breadcrumb')
    ...
    <li class="inline-flex items-center">
        <i class="fas fa-chevron-right text-gray-400 mx-2 text-xs"></i>
        <span class="text-gray-500 font-medium">Kategori Pengeluaran</span>
    </li>
@endsection
...
@section('content')
    <div class="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
        <div class="px-6 py-4 border-b border-gray-200 flex flex-wrap items-center justify-between gap-3">
            <div>
                <h5 class="text-sm font-semibold text-gray-700">Daftar Kategori Pengeluaran</h5>
                <p class="text-xs text-gray-400 mt-0.5">Kelola semua kategori pengeluaran</p>
            </div>
            <a href="{{ route('master.kategori-pengeluaran.create') }}"
        ...
        <div class="p-6">
            ...
            <div id="table-container">
                @include('kategori-pengeluarans.table')
            </div>
        </div>
    </div>
@endsection
...
@push('scripts')
    <script>
        function applyFilter() {
            ...
            fetch(`{{ route('master.kategori-pengeluaran.index') }}?search=${search}&ajax=true`)
