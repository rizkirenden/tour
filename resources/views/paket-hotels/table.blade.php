    ...
    <table class="w-full text-sm">
        <thead>
            <tr class="border-b border-gray-200">
                ...
                <th class="px-4 py-3 text-center text-xs font-semibold text-gray-500 uppercase tracking-wider">Urutan
                </th>
                <th class="px-4 py-3 text-center text-xs font-semibold text-gray-500 uppercase tracking-wider">Default
                </th>
                <th class="px-4 py-3 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider">Tipe
                    Penginapan</th>
        ...
        <tbody class="divide-y divide-gray-100">
            @forelse($data as $index => $item)
                <tr class="hover:bg-gray-50 transition-colors duration-150">
                    ...
                    <td class="px-4 py-3">
                        <p class="font-medium text-gray-800">{{ $item->hotel->nama_hotel ?? '-' }}</p>
                        <p class="text-xs text-gray-400">{{ $item->hotel->kota ?? '' }},
                            {{ $item->hotel->negara ?? '' }}</p>
                    </td>
                    ...
                    <td class="px-4 py-3">
                        <span class="text-sm text-gray-600">{{ $item->tipe_penginapan ?? '-' }}</span>
                    </td>
                    ...
                    <td class="px-4 py-3">
                        <div class="flex items-center justify-center gap-1.5">
                            <a href="{{ route('master.paket-hotel.show', $item->id_paket_hotel) }}"
                                class="p-2 text-blue-500 hover:bg-blue-50 rounded-lg transition-all duration-200">
                                <i class="fas fa-eye text-sm"></i>
                            </a>
                            <a href="{{ route('master.paket-hotel.edit', $item->id_paket_hotel) }}"
                                class="p-2 text-amber-500 hover:bg-amber-50 rounded-lg transition-all duration-200">
                                <i class="fas fa-edit text-sm"></i>
                            </a>
                            <button onclick="confirmDelete({{ $item->id_paket_hotel }})"
                                class="p-2 text-red-500 hover:bg-red-50 rounded-lg transition-all duration-200">
                                <i class="fas fa-trash text-sm"></i>
                            </button>
                            <form id="delete-form-{{ $item->id_paket_hotel }}"
                                action="{{ route('master.paket-hotel.destroy', $item->id_paket_hotel) }}"
                ...
                <tr>
                    <td colspan="8" class="px-4 py-12 text-center">
                        <div class="flex flex-col items-center">
                            ...
                            <p class="text-gray-500 font-medium">Belum ada data paket hotel</p>
                            <p class="text-gray-400 text-sm mt-1">Klik tombol "Tambah Paket Hotel" untuk menambahkan</p>
                        </div>
                    </td>
                </tr>
            @endforelse
...
@push('scripts')
    <script>
        function confirmDelete(id) {
            if (confirm('Yakin ingin menghapus paket hotel ini?')) {
