...
class Departure extends Model
{
    ...
    public function getTotalHppFormattedAttribute()
    {
    ...
    {
        return number_format($this->margin_laba, 2) . '%';
    }
    ...
    {
        $jamaahs = $this->jamaahs;
        $totalJamaah = $jamaahs->count();

        $totalPendapatanKotor = 0;
        foreach ($jamaahs as $jamaah) {
            $totalPendapatanKotor += $jamaah->total_tagihan_sebelum_diskon ?? 0;
        }
        ...
        $totalHpp = $this->calculateHpp($totalJamaah);
        $labaBersih = $totalPendapatanBersih - $totalHpp;
        $marginLaba = $totalPendapatanBersih > 0 ? ($labaBersih / $totalPendapatanBersih) * 100 : 0;

        $this->update([
            'jamaah_terdaftar' => $totalJamaah,
            'total_pendapatan_kotor' => $totalPendapatanKotor,
            'total_diskon' => $totalDiskon,
            'total_pendapatan_bersih' => $totalPendapatanBersih,
            'total_hpp' => $totalHpp,
            'laba_bersih' => $labaBersih,
            'margin_laba' => round($marginLaba, 2),
        ]);

        return $this;
    }

   public function calculateHpp($totalJamaah = null)
{
    if ($totalJamaah === null) {
    ...
    foreach ($this->jamaahs as $jamaah) {
        $totalHpp += ($jamaah->total_tiket_domestik ?? 0) +
                     ($jamaah->total_tiket_international ?? 0) +
                     ($jamaah->fee_agent ?? 0);
    }

    $totalHpp += ($this->harga_maskapai_domestik_berangkat ?? 0) +
                 ($this->harga_maskapai_domestik_pulang ?? 0) +
                 ($this->harga_maskapai_internasional_berangkat ?? 0) +
                 ($this->harga_maskapai_internasional_pulang ?? 0);

    $totalHpp += $this->total_harga_hotel_mekkah +
                 $this->total_harga_hotel_madinah +
                 $this->total_harga_hotel_transit;

    $totalHpp += $this->total_harga_perlengkapan;

    // Tambahkan jenis transaksi ke HPP
    $totalHpp += $this->total_jenis_transaksi;

    // ==========================================
    // TAMBAHKAN INI - PAKET TOUR HOTEL
    // ==========================================
    $totalHpp += $this->total_harga_paket_tour_hotel;
...
// Accessor untuk total harga hotel tour
